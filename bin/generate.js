...
function combineKeyAndProperty(key, property, separator) {
    return pipeable_1.pipe(getType(property), E.map(function (type) { return "" + key + separator + type; }));
}
function getTypeSchemas(separator) {
    return function (schemas) {
        return pipeable_1.pipe(A.array.traverse(E.either)(Object.entries(schemas), function (_a) {
            var key = _a[0], property = _a[1];
            return combineKeyAndProperty(key, property, separator);
        }));
    };
}
function getType(property) {
    ...
    if (property.type === "array") {
        return pipeable_1.pipe(property.items, getType, E.map(function (type) { return "Array<" + type + ">"; }));
    }
    if (property.type === "string" && property.enum) {
        return E.right(property.enum.map(function (enumValue) { return "'" + enumValue + "'"; }).join(" | "));
    }
    if ("allOf" in property && property.allOf) {
        return pipeable_1.pipe(A.array.traverse(E.either)(property.allOf, getType), E.map(function (types) { return types.join(" & "); }));
    }
    if ("anyOf" in property && property.anyOf) {
        return pipeable_1.pipe(A.array.traverse(E.either)(property.anyOf, getType), E.map(function (types) { return types.join(" | "); }));
    }
    if ("oneOf" in property && property.oneOf) {
        return pipeable_1.pipe(A.array.traverse(E.either)(property.oneOf, getType), E.map(function (types) { return types.join(" | "); }));
    }
...
function generate(swagger) {
    return pipeable_1.pipe(swagger, getDefinitions, E.chain(getTypeSchemas("=")), E.map(function (properties) { return properties.map(function (prop) { return "type " + prop; }).join(";"); }));
}
